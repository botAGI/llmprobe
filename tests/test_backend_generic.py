...
from llmprobe.backends.generic import detect, read_config
...
@pytest.mark.asyncio
async def test_read_config_survives_missing_or_malformed_models() -> None:
    """A missing /v1/models or a malformed body yields empty model id, no raise."""
    for status, body in ((404, None), (500, None), (200, {"unexpected": True})):
        client = _client(models_status=status, body=body)
        try:
            config = await read_config(client, BASE_URL)
        finally:
            await client.aclose()
