...
def main(
    ...
    if base_url is None:
        # One command, no arguments (the README's `uvx llmprobe` discovery step):
        # show help and exit 0 rather than failing with "BASE_URL is required".
        click_group = typer.main.get_command(app)
        print(click_group.get_help(click_group.make_context("llmprobe", [])))
        raise typer.Exit(code=0)
    try:
        report = asyncio.run(
            probe(
